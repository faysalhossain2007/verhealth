...
from src.paraphraseGenerator.paraphrasermaster.paraphraser.embeddings import load_sentence_embeddings
from src.paraphraseGenerator.paraphrasermaster.paraphraser.preprocess_data import preprocess_batch
from six.moves import input
from src.paraphraseGenerator.paraphrasermaster.paraphraser.lstm_model import lstm_model
...
def get_paraphraser():
    paraphraser = Paraphraser(PARAPHRASE_MODEL)
    return paraphraser

def generate_paraphrase(source_sentence, sampling_temp, how_many, paraphraser):

    paraphrases = paraphraser.sample_paraphrase(source_sentence, sampling_temp=sampling_temp, how_many=how_many)
    return paraphrases
...
def run():
    ...
    sents = generate_paraphrase("i faced an accident in 2013", 0.99, 100, p)
    for sent in sents:
        print(sent)

    sents = generate_paraphrase("i faced an accident in 2013", 0.80, 100, p)
    for sent in sents:
        print(sent)

    # sents = generate_paraphrase("i have change in behavior.", 0.80, 10, p)
    # for sent in sents:
    #     print(sent)
